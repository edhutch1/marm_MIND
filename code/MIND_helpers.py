...
def calculate_mind_network(data_df, feature_cols, region_list, resample=False, n_samples = 4000):
    ...
    if (len(feature_cols) > 1) and resample==True: 
        ...
        for name, group in data_df.groupby('Label'):
            resampled_values = stats.gaussian_kde(group[feature_cols].T.values).resample(n_samples)

            resampled_df = pd.DataFrame(resampled_values.T, columns=feature_cols)
            resampled_df['Label'] = name
            resampled_dataset = pd.concat([resampled_dataset, resampled_df], ignore_index=True)
